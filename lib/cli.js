...
exports.run = function (options) {
    ...
    var processLog = function (value, callback) {
        ...
        getStats(config.log, function (error, result) {
            ...
            var fileStream = Fs.createReadStream(config.log, {
                encoding: 'utf8',
                start: value.start
            });
            var transform = new Utils.Liner();

            transform.on('readable', function (line) {
            ...
            transform.on('end', function () {
                ...
                value.start += this.bytesTransformed + 1;
                ...
                var tasks = [];
                tasks.push(broadcast.bind(null, events, {
                    url: config.url,
